...
unsigned long RunTestForExponent(int numExpBit, FILE* f, char* FuncName) {
  ...
  for (unsigned bitlen = numExpBit + 24; bitlen <= numExpBit + 24; bitlen++) {
    ...
    // Run at most 64K at a time. That's still 4 * 22 * 7 * 64K = 50M tests
    unsigned long upperlimit = 1lu << (unsigned long)bitlen;
    for (unsigned long count = 0; count < upperlimit; count += 1) {
      if (count % 0x100000 == 0) {
	printf("count = 0x%lx (%lu)\r", count, wrongResult);
	fflush(stdout);
      }
