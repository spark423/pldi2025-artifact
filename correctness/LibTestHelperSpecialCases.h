...
unsigned long RunTestForSpecialCases(FILE* f, char* FuncName, unsigned* special_cases) {
  unsigned long totalWrongResult = 0;
  unsigned long wrongResult = 0;
  int bias = 127;
  int emax = 128;
  ...
  mpfr_init2(mval, 24);
  unsigned long special_case_number = sizeof(special_cases)/sizeof(unsigned);
  for (int rnd_index = 0; rnd_index < 4; rnd_index++) {
    fesetround(fenv_rnd_modes[rnd_index]);
    for (unsigned long i = 0; i < special_case_number ; i++) {
      float x = ConvertBinToFP(special_cases[i], 8, 32); 
      float_x oracleResult = {.f = MpfrResult(x, 8, 32, mpfr_rnd_modes[rnd_index])};
      double res = __ELEM__(x);
      float_x roundedResult = {.f = RoundDoubleToFEN(res, 8, 32, my_rnd_modes[rnd_index], 0)};
      if (oracleResult.f != oracleResult.f && roundedResult.f != roundedResult.f) continue;
      if (oracleResult.x != roundedResult.x && wrongResult < 10) wrongResult++;
    }
  } 
  if (wrongResult == 0) fprintf(f, "Testing FP32(8 exp bit): check    \n");
  else fprintf(f, "Testing FP32(8 exp bit): incorrect\n");
  fflush(f);
  totalWrongResult += wrongResult;
  ...
  if (totalWrongResult == 0) {
    fprintf(f, "FP reps with 8 exp bits: check    \n");
  } else {
    fprintf(f, "FP reps with 8 exp bits: incorrect\n");
  }
  if (totalWrongResult == 0) {
    printf("FP reps with 8 exp bits: \033[0;32mcheck\033[0m    \n");
  } else {
    printf("FP reps with 8 exp bits: \033[0;31mincorrect\033[0m\n");
  }
  return totalWrongResult;
}

void RunTest(char* logFile, char* FuncName, unsigned* special_cases) {
  ...
  RunTestForSpecialCases(f, FuncName, special_cases);
  fclose(f);
}
