...
unsigned long RunTestOracle(FILE* f, char* FuncName, unsigned* special_cases) {
  float_x x;
  ...
  for (unsigned long count = 0x0; count < special_case_number; count += 1) {
    x.x = special_cases[count];
    double_x oracleResult = {.d = ComputeOracleResult(x.f, mval)};
    int all_correct = 1;
    for (int rnd_index = 0; rnd_index < 4; rnd_index++) {
      fesetround(fenv_rnd_modes[rnd_index]);
...
void RunTest(char* logFile, char* FuncName, unsigned* special_cases) {
  ...
  printf("Function: %s\n", FuncName);
  RunTestOracle(f, FuncName, special_cases);
  fclose(f);
}
