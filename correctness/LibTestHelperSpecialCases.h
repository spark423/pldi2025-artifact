...
void RunTestOracle(unsigned* special_cases) {
  for (unsigned bitlen = 10; bitlen <= 32; bitlen++) {
    ...
    int special_cases_count = sizeof(special_cases)/sizeof(unsigned);
    if (bitlen == 32) { 
      for (int i = 0x0; i < special_cases_count; i++) {
	int all_correct = 1;
	float_x x = {.f=ConvertBinToFP((unsigned)special_cases[i], 8, bitlen)};
	double oracle_res = ComputeOracleResult(x.f, mval);
	for (int rnd_index = 0; rnd_index < 4; rnd_index++) {
	  float_x oracleResult = {.f=RoundDoubleToF8N(oracle_res, bitlen, my_rnd_modes[rnd_index])};
	  fesetround(fenv_rnd_modes[rnd_index]);
...
void RunTest(char* FuncName, unsigned* special_cases) {
  mpfr_init2(mval, 25);
  mpfr_set_emin(-150);
  mpfr_set_emax(128);
  printf("Function: %s\n", FuncName);
  RunTestOracle(special_cases);
}
