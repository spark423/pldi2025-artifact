...
double ourlibm_coshf_fma(float x) {
  float_x fx;
  fx.f = x;
  fx.x &= 0x7fffffff;
  ...
  double sinhL = __builtin_fma(R2, 0x1.116adea7782bep-7, 0x1.555555299a90fp-3);
  sinhL = R*__builtin_fma(sinhL, R2, 0x1.0000000000065p+0);
  double coshL;
  if (__builtin_expect(R == 0x1.d2fd8bccca2c4p-9, 0)) coshL = 0x1.00006a22acaffp+0;
  else {
    //0x1.d2fd8bccca2c4p-9
    //y=0x1.0000000000002p+0 x^(0) + 0x1.fffffffdbb4f2p-2 x^(2) + 0x1.555833375d5e2p-5 x^(4)
    coshL = __builtin_fma(R2, 0x1.555833375d5e2p-5, 0x1.fffffffdbb4f2p-2);
    coshL = __builtin_fma(coshL, R2, 0x1.0000000000002p+0);
  }
  return __builtin_fma(sinhHM, sinhL, coshHM*coshL);
}
