...
int main(int argc, char** argv) {
  if (argc != 4) {
    printf("Usage: %s <Name of the Cos Interval File 1> <Name of the Cos Interval File 2> <Oracle File>\n", argv[0]);
    exit(0);
  }
  ...
  double y;
  unsigned long count;
  for (count=0; count<0x39800001; ++count) {
  ...
  for (count=0x39800001; count<0x3C490FDB; ++count) {
    size_t size = fread(&y, sizeof(double), 1, fp_oracle); 
    if (y == 0.0 || y == 1.0 || y == -1.0) {
      continue;
    }
